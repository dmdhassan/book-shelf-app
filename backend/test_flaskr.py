...
class BookTestCase(unittest.TestCase):
    ...
    def test_get_search_books(self):
        ...
        self.assertEqual(res.status_code, 200)
        self.assertEqual(data['success'], True)
        self.assertTrue(len(data['search']), 4)

    def test_404_search_not_found(self):
        res = self.client().post('/books/', json={'search': 'salisu'})
        data = json.loads(res.data)

        self.assertEqual(res.status_code, 200)
        self.assertEqual(data['success'], True)
        self.assertTrue(len(data['search']), 0)
    ...
    def test_update_book_rating(self):
        ...
        self.assertEqual(res.status_code, 200)
        self.assertEqual(data['success'], True)
        self.assertEquual(book.format()['rating'], 1)
    ...
    def test_405_if_book_creation_not_allowed(self):
        res = self.client().post('/book/45', json=self.new_book)
        data = json.loads(res.data)
